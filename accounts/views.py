          ...
          if user is not None :
              try:
                 if ( user.patient.is_patient == True ) :
                  ...
                  messages.info(request,'Please enter the correct username and password for patient account.')
                  return redirect('sign_in_patient')
          else :
             messages.info(request,'Please enter the correct username and password for patient account.')
             return redirect('sign_in_patient')
    else :
            return render(request,'patient/signin_page/patient.html')
...
def signup_doctor(request):
    ...
    if request.method == 'POST':
      if request.POST['username'] and request.POST['email'] and  request.POST['name'] and request.POST['dob'] and request.POST['gender'] and request.POST['address']and request.POST['mobile'] and request.POST['password']and request.POST['password1']  and  request.POST['registration_no'] and  request.POST['specialization'] :
          username =  request.POST['username']
          email =  request.POST['email']
          name =  request.POST['name']
          dob =  request.POST['dob']
          ...
          year_of_registration =  date.today()
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                doctornew = doctor( user=user, name=name, dob=dob, gender=gender, address=address, mobile_no=mobile_no, registration_no=registration_no, year_of_registration=year_of_registration, specialization=specialization )
                doctornew.save()
                messages.info(request,'user created sucessfully')
...
def saveddata(request,doctorusername):
  if request.method == 'POST':
    ...
    doctor.objects.filter(pk=duser.doctor).update( name=name, dob=dob, gender=gender, address=address, mobile_no=mobile_no, registration_no=registration_no, year_of_registration=yor, specialization=specialization )
